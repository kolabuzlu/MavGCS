...
// Where on the aeroplane the coordinate actually is.
//
// This used to be half the icon's height, which is not a point on the
// aircraft at all - it is the middle of the picture. The artwork is
// centred in its canvas and the tail is long, so that landed on the
// wing's trailing edge: about six screen pixels behind where the
// aeroplane reads as being. Against open map it is invisible; against a
// waypoint or a fence line it is not, which is where it was noticed.
//
// Row 253 of the 605px source is the widest part of the drawing - the
// wing line - and that is what the eye takes as the aircraft's position.
var DRONE_ANCHOR_FRAC = 253 / 605;              // 0.418 down the icon
var DRONE_ANCHOR_Y = DRONE_ICON_H * DRONE_ANCHOR_FRAC;   // 32.6 of 78
var droneIcon = L.divIcon({
    className: 'drone-icon',
    html: '<div style="width:' + DRONE_ICON_W + 'px;height:' + DRONE_ICON_H +
          'px;transform-origin:50% ' + (DRONE_ANCHOR_FRAC * 100).toFixed(3) +
          '%;">' +
    ...
    iconAnchor: [DRONE_ICON_W / 2, DRONE_ANCHOR_Y]
});
...
// The opening view gets the same treatment. HOME_LAT/HOME_LON went to the
// widget's geometric centre, which puts the starting coordinate under the
// instrument column - the same complaint the aircraft had, in the one view
// you see before any telemetry arrives.
//
// Done down here rather than beside HOME_LAT because FOLLOW_INSET does not
